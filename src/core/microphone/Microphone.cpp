...
MicrophoneInputAudioData *Microphone::start() {
  auto err = Pa_OpenDefaultStream(&m_stream, m_config.channels,
                                  0, // Output channels
                                  paFloat32, m_config.sampleRate, 512,
                                  microphoneCallback, &m_data);
  if (err != paNoError) {
    std::ostringstream oss;
...
int Microphone::microphoneCallback(const void *inputBuffer, void *outputBuffer,
  ...
  auto *data = static_cast<MicrophoneInputAudioData *>(userData);
