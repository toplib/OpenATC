...
int main(int argc, char *argv[]) {
    ...
    STT::WhisperConfig whisper_config = {.model = "/home/toplib/CLionProjects/SimpleATC/models/ggml-large-v3-turbo-q8_0.bin"};
    ...
    while (true) {
        ...
        for (float sample : samples) {
            ...
            if (frame.size() >= frame_size) {
                bool is_speech = WebRtcVad_Process(vad, 16000, frame.data(), frame_size) == 1;
                ...
                if (is_speech) {
                    ...
                    if (silence_frames > silence_frames_threshold) {
                        if (speech_frames >= min_speech_frames && speech_buffer.size() >= 16000) {
